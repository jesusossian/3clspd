...
function FixAndOptimizeStandardFormulation(inst::InstanceData, params, SETR, SETW, SETP, bestsol)
	...
	nbsubprob = ceil(inst.NT/params.fixsizefo)
	...
	if params.solver == "Gurobi"
		env = Gurobi.Env()
		model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=maxtimesubprob, MIPGap=params.tolgapfo))
	#elseif params.solver == "Cplex"
	#	model = Model(solver=CplexSolver(CPX_PARAM_TILIM=maxtimesubprob, CPX_PARAM_EPGAP=params.tolgapfo))
	else
	...
	for t in 1:inst.NT
		set_start_value(yp[1,t],ypsol[1,t])
	end

	for w in 1:inst.NW, t in 1:inst.NT
		set_start_value(yw[w,t],ywsol[w,t])
	end

	for r in 1:inst.NR, t in 1:inst.NT
		set_start_value(yr[r,t],yrsol[r,t])
	end
...
function lpbasedimprovement(inst::InstanceData, params, SETR, SETW, SETP, bestsol)
	...
	if params.solver == "Gurobi"
		env = Gurobi.Env()
		model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=params.maxtime))
	#elseif params.solver == "Cplex"
	...
	plantcost = 0
	for t in 1:inst.NT
		plantcost += inst.SCP[1,t]*value(yp[1,t]) + inst.HCP[1]*value(sp[1,t])
 	end
	println("Plant cost = $(plantcost)")

	warecosts = 0
	for w in 1:inst.NW, t in 1:inst.NT
		warecosts += inst.SCW[w,t]*value(yw[w,t]) + inst.HCW[w]*value(sw[w,t])
	end
	println("Warehouses cost = $(warecosts)")

	retcosts = 0
	for r in 1:inst.NR, t in 1:inst.NT
		retcosts += inst.SCR[r,t]*value(yr[r,t]) + inst.HCR[r]*value(sr[r,t])
	end
	...
	for t in 1:inst.NT
		if value(yp[1,t]) > 0.9999 && value(xp[1,t]) < 0.0001
			println("plant, period $(t)")
		end
 	end

	for w in 1:inst.NW, t in 1:inst.NT
		if value(yw[w,t]) > 0.9999 && value(xw[w,t]) < 0.0001
			println("warehouse $(w), period $(t)")
		end
	end

	for r in 1:inst.NR, t in 1:inst.NT
		if value(yr[r,t]) > 0.9999 && value(xr[r,t]) < 0.0001
...
function FixAndOptimizeEchelonStockFormulation2(inst::InstanceData, params, SETR, SETW, SETP, bestsol)
	...
	println("Running FixAndOptimize.FixAndOptimizeEchelonStockFormulation2")
...
function FixAndOptimizeEchelonStockFormulation3(inst::InstanceData,params,SETR,SETW,SETP,bestsol,constructiveelapsed)

	freeinterval = params.freeintervalfo

	toltime = 1

	fixsizefo = params.fixsizefo
	horsizefo = fixsizefo + freeinterval
	tolgapfo = params.tolgapfo

	maxfixsizefo = inst.NT - freeinterval
	maxhorsizefo = inst.NT

	if params.maxtimefo < 3600
		maxtimefo = params.maxtimefo - constructiveelapsed
	else
		maxtimefo = 300
	end
	println("maxtimefo = $(maxtimefo)")

	#Defining number of subproblems and maximum time for executing them
	nbsubprob = ceil(inst.NT/fixsizefo)
	#maxtimesubprob = floor(maxtimefo/(2*nbsubprob)) #try to guarantee at least two passes
	maxtimesubprob = floor(maxtimefo/(params.minroundsfo*nbsubprob)) #try to guarantee at least two passes
	...
	println("Running FixAndOptimize.FixAndOptimizeEchelonStockFormulation3")
	if params.solver == "Gurobi"
		if !(@isdefined env)
		    env = Gurobi.Env()
		end
		println("defined env")
		#model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=maxtimesubprob,MIPGap=tolgapfo,OutputFlag=0))
		model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=maxtimesubprob,MIPGap=tolgapfo))
	#elseif params.solver == "Cplex"
	#	model = Model(solver=CplexSolver(CPX_PARAM_TILIM=maxtimesubprob,CPX_PARAM_EPGAP=tolgapfo))
	else
		println("No solver selected")
	...
	#define feasible solution to be offered as input
	for t in 1:inst.NT
		set_start_value(yp[1,t],ypsol[1,t])
	end
	...
	#min and max sizes of solving intervals
	mink = horsizefo
	maxk = maxhorsizefo

	#min and max sizes of fixing intervals
	minkprime = fixsizefo
	maxkprime = maxfixsizefo
	...
	while keepfo == true
		...
		println(kprime,"  ",maxkprime)
		println(k," ",kprime+1," <? ",maxk)
		k = kprime+freeinterval
		#for it in max(k,kprime+1):maxk
			println("\n\n ########### RESTARTING FIX AND OPTIMIZE with k=$(k) and kprime=$(kprime)")
			proceed = true
			while (proceed && elapsedtime + maxtimesubprob <= maxtimefo+toltime )
				proceed = false
				...
				while(beta <= inst.NT && elapsedtime + maxtimesubprob <= maxtimefo+toltime)
					...
					if elapsedtime <= maxtimefo - 0.5 && elapsedtime + maxtimesubprob > maxtimefo+toltime
						println("\n\n\n ##################### \n UPDATING MAXTIMESUBPROB \n\n\n")
						set_optimizer_attributes(model,
                                    "TimeLimit" => maxtimefo - elapsedtime)
						maxtimesubprob = maxtimefo - elapsedtime
						println("maxtimesubprob = $maxtimesubprob")
					end
		...
		elseif elapsedtime+maxtimesubprob < maxtimefo + toltime && kprime < maxkprime
			keepfo = true
			kprime += 1
		end
	...
	elapsedtime += constructiveelapsed
	open("saida.txt","a") do f
		write(f,";$(params.fixsizefo);$(params.freeintervalfo);$(params.minroundsfo);$(params.maxtimerf);$(bestsol);$(elapsedtime);$rounds\n")
	end
...
function FixAndOptimizeEchelonStockESLS(inst::InstanceData,params,SETR,SETW,SETP,bestsol,constructiveelapsed)
