...
function RandomizedDPHeuristicBottomUp(inst,params)
	...
	#while elapsedtime < params.dpmaxiter
		if iter != 1
...
function RandomizedDPHeuristicBottomUpCC(inst,params)
	...
	#for iter in 1:params.dpmaxiter
	while elapsedtime < params.dpmaxtime
		iter += 1
		if params.dptype == 1
		...
		if feasibleCapacitatedProblem(newDP,inst.NT,inst.C[1])
			...
			if calcsolcost < bestcost && elapsedtime < params.dpmaxiter
				bestcost = calcsolcost
				bestSETR = SETR
				bestSETW =SETW
				bestSETP = SETP
	...
	if feasiblerounds > 0
		averagecost = sumcosts/feasiblerounds
	end
	percentagefeasible = 100*feasiblerounds/iter
	...
	println("Rounds = ", iter)
	println("Feasible rounds = ",percentagefeasible)
	...
	if params.form == "randdpheurCCfo" || params.form == "lr"
		...
		bestcost = FixAndOptimize.FixAndOptimizeEchelonStockFormulation3(inst,params,bestSETR,bestSETW,bestSETP,bestcost,elapsedtime)
		t2 = time_ns()
	...
	if params.form == "randdpheurCC"
		open("saida.txt","a") do f
			write(f,";$(inst.NR);$(inst.NT);$(inst.NW);$(params.capacity);$(params.balanced);$bestcost;$averagecost;$elapsedtime;$(params.dptype);$(params.dpalpha);$(params.dpredperiods);$(params.dpcostreductionware);$(params.dpcostreductionret);$(params.dpseed);$(iter);$(percentagefeasible) \n")
		end
	elseif params.form == "randdpheurCCfo"
			open("saida.txt","a") do f
				write(f,";$(inst.NR);$(inst.NT);$(inst.NW);$(params.capacity);$(params.balanced);$bestcostdpheur;$averagecost;$elapsedtime;$(params.dptype);$(params.dpalpha);$(params.dpredperiods);$(params.dpcostreductionware);$(params.dpcostreductionret);$(params.dpcostreductionret);$(iter);$(percentagefeasible);$bestcost;$elapsedtimefo \n")
			end
