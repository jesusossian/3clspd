...
function standardFormulation(inst::InstanceData, params::ParameterData)
	...
	if params.capacityw != 0.0
		@constraint(model,
			capW[w=1:inst.NW,t=1:inst.NT], sw[w,t] <= inst.CW[w,t]
		)
	end

	if params.capacityr != 0.0
		@constraint(model,
			capR[r=1:inst.NR,t=1:inst.NT], sr[r,t] <= inst.CR[r,t]
		)
	end
	...
	open("saida.txt","a") do f
		write(f,";$(params.form);$(params.capacity);$(params.capacityw);$(params.capacityr);$(inst.NT);$(inst.NW);$(inst.NR);$bestbound;$bestsol;$gap;$time;$numnodes;$opt \n")
	end
...
function multicommodityFormulation(inst::InstanceData, params::ParameterData)
	...
	if params.solver == "Gurobi"
		if params.disablesolver == 1 #Disable gurobi cuts and presolve
			if params.maxnodes < 999.0
				model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=params.maxtime,MIPGap=params.tolgap,CliqueCuts=0, CoverCuts=0, FlowCoverCuts=0,FlowPathCuts=0,MIRCuts=0,NetworkCuts=0,GomoryPasses=0, PreCrush=1,NodeLimit=params.maxnodes,Method=2))
			else
				model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=params.maxtime,MIPGap=params.tolgap,CliqueCuts=0, CoverCuts=0, FlowCoverCuts=0,FlowPathCuts=0,MIRCuts=0,NetworkCuts=0,GomoryPasses=0, PreCrush=1,Method=2))
			end

		else
			if params.maxnodes < 999.0
				model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=params.maxtime, MIPGap=params.tolgap, PreCrush=1, NodeLimit=params.maxnodes, Method=2))
			else
				model = Model(with_optimizer(Gurobi.Optimizer,TimeLimit=params.maxtime, MIPGap=params.tolgap, PreCrush=1, Method=2))
			end
		end
	#elseif params.solver == "Cplex"
	#	model = Model(solver = CplexSolver(CPX_PARAM_TILIM=params.maxtime,CPX_PARAM_EPGAP=params.tolgap))
	...
	### Capacity constraints ###
	if params.capacity != 0.0
		@constraint(model,capP[t=1:inst.NT], sum(xxp[r,t,k] for r=1:inst.NR,k=t:inst.NT) <= min(inst.C[t],sum(inst.DP[1,k] for k in t:inst.NT))*yp[1,t]
        	)
	end

	if params.capacityw != 0.0
		@constraint(model,
			capW[w=1:inst.NW,t=1:inst.NT], sum(ssw[inst.DeltaW[w][r1],k,kl] for r1=1:length(inst.DeltaW[w]),k=1:t,kl= t+1:inst.NT) <= inst.CW[w,t]
		)
	end

	if params.capacityr != 0.0
		@constraint(model,
			capR[r=1:inst.NR,t=1:inst.NT], sum(ssr[r,k,kl] for k=1:t,kl= t+1:inst.NT) <= inst.CR[r,t]
		)
	end
...
function echelonStockFormulationESN(inst::InstanceData, params::ParameterData)
	...
	if params.capacity != 0.0
		@constraint(model,
			capP[t=1:inst.NT],sum(zp[1,t,k]*sdp[1,t,k] for k in t:inst.NT) <= min(inst.C[t],sum(inst.DP[1,k] for k in t:inst.NT))*yp[1,t]
		)
	end
	...
	### Balance constraints ###
	@constraint(model,
		balanceP[i=1:inst.NP],sum(zp[i,1,k] for k in 1:inst.NT) == 1
	)
	@constraint(model,
